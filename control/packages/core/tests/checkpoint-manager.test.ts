...
describe('hasChanges', () => {
  it('returns true when git status reports changes', () => {
    mockSpawn.mockReturnValue({ status: 0, stdout: ' M src/app.ts\n', stderr: '' });
    expect(hasChanges('/tmp/worktree')).toBe(true);
    expect(mockSpawn).toHaveBeenCalledWith('git', ['status', '--porcelain'], { cwd: '/tmp/worktree' });
  });
...
describe('createCheckpoint', () => {
  ...
  it('throws when git add fails', () => {
    mockSpawn
      .mockReturnValueOnce({ status: 0, stdout: ' M file.ts\n', stderr: '' })
      .mockReturnValueOnce({ status: 1, stdout: '', stderr: 'error: could not add' });
  ...
  it('throws when git commit fails', () => {
    mockSpawn
      .mockReturnValueOnce({ status: 0, stdout: ' M file.ts\n', stderr: '' })
      .mockReturnValueOnce({ status: 0, stdout: '', stderr: '' })
      .mockReturnValueOnce({ status: 1, stdout: '', stderr: 'error: commit failed' });
...
describe('parseTimelineOutput', () => {
  ...
  it('handles summary containing pipe characters', () => {
    const output = 'abc1234|2025-03-15T10:30:00+00:00|Updated colors | blue to red\n';
    const entries = parseTimelineOutput(output);
...
describe('getTimeline', () => {
  it('returns timeline entries from git log', () => {
    ...
    const entries = getTimeline({ worktreePath: '/tmp/worktree', baseRef: 'abc000' });
    expect(entries).toHaveLength(1);
  ...
  it('returns empty array when git log fails', () => {
    mockSpawn.mockReturnValue({ status: 1, stdout: '', stderr: 'fatal: bad ref' });
    const entries = getTimeline({ worktreePath: '/tmp/worktree', baseRef: 'badref' });
...
describe('rollbackToCheckpoint', () => {
  it('resets HEAD to the target commit', () => {
    ...
    rollbackToCheckpoint({ worktreePath: '/tmp/worktree', targetHash: 'abc1234' });
  ...
  it('throws when git reset fails', () => {
    mockSpawn.mockReturnValue({ status: 1, stdout: '', stderr: 'fatal: bad revision' });
    expect(() =>
      rollbackToCheckpoint({ worktreePath: '/tmp/worktree', targetHash: 'badref' }),
...
describe('checkpoint-manager — negative paths', () => {
  it('createCheckpoint throws when git rev-parse fails after successful commit', () => {
    mockSpawn
      ...
      .mockReturnValueOnce({ status: 0, stdout: '', stderr: '' })
      .mockReturnValueOnce({ status: 0, stdout: '', stderr: '' })
      .mockReturnValueOnce({ status: 1, stdout: '', stderr: 'fatal: ambiguous' });
