    ...
    literals: Object.fromEntries(
      Object.keys(spec.literals).map((key) => {
        ...
        if (k.includes('DATABASE_URL')) {
          ...
          const db = k.includes('AUDIT') ? 'calypso_audit'
            : k.includes('ANALYTICS') ? 'calypso_analytics'
            : pgDb;
        ...
        if (k === 'AUDIT_DB') return [key, 'calypso_audit'];
        if (k === 'ANALYTICS_DB') return [key, 'calypso_analytics'];

        // URL keys → placeholder URL.
        if (k.includes('BASE_URL') || k.includes('API_URL')) {
          return [key, 'http://calypso-app:31415'];
        }
...
export function seedApplicationData(
  ...
  const pgUser = pgSecret.literals['POSTGRES_USER'] ?? 'calypso';
  const pgDb = pgSecret.literals['POSTGRES_DB'] ?? 'calypso';

  // Find the postgres pod (StatefulSet pod naming convention: <name>-0).
  const podResult = spawn('kubectl', [
    'get', 'pods', '-l', 'app=calypso-postgres',
    `--namespace=${config.namespace}`,
