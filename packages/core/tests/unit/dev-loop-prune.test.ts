...
async function makeWorktreeDir(issueNumber: number, slug: string): Promise<string> {
...
describe('runPrunePass — worktree cleanup', () => {
  it('prunes worktrees for closed issues', async () => {
    ...
    const client = makeClient({
      getIssue: vi.fn().mockResolvedValue(makeIssue({ number: 10, state: 'closed' })),
    ...
    expect(await fs.stat(wtPath).then(() => true).catch(() => false)).toBe(false);
  });
  ...
  it('leaves worktrees for open issues intact', async () => {
    ...
    const client = makeClient({
      getIssue: vi.fn().mockResolvedValue(makeIssue({ number: 10, state: 'open' })),
      listIssues: vi.fn().mockResolvedValue([]),
    });
...
describe('runPrunePass — stale session reaping', () => {
  it('deletes session comments older than the stale timeout', async () => {
    ...
    const client = makeClient({
      ...
      listIssueComments: vi.fn().mockResolvedValue([{ id: 99, body: sessionBody }]),
      deleteIssueComment: vi.fn().mockResolvedValue(undefined),
    });
  ...
  it('ignores issues with no session comment', async () => {
    const client = makeClient({
      listIssues: vi.fn().mockResolvedValue([makeIssue({ number: 10 })]),
      listIssueComments: vi.fn().mockResolvedValue([{ id: 99, body: 'regular comment' }]),
      deleteIssueComment: vi.fn().mockResolvedValue(undefined),
    });
