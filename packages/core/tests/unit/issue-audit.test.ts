...
describe('runIssueAudit', () => {
  it('returns empty result when no candidate issues', async () => {
    ...
    const result = await runIssueAudit(client, 'org', 'repo', { spawn: fakeSpawn(conformantResponse) });
  ...
  it('posts findings comment on non-conformant issues', async () => {
    ...
    const result = await runIssueAudit(client, 'org', 'repo', { spawn: fakeSpawn(nonConformantResponse) });
  ...
  it('updates existing audit comment instead of creating duplicate', async () => {
    ...
    await runIssueAudit(client, 'org', 'repo', { spawn: fakeSpawn(nonConformantResponse) });
    expect(client.updateIssueComment).toHaveBeenCalledWith('org', 'repo', 500, expect.any(String));
    expect(client.createIssueComment).not.toHaveBeenCalled();
  });
  ...
  it('throws when LLM response is missing required fields', async () => {
    ...
    await expect(
      runIssueAudit(client, 'org', 'repo', { spawn: fakeSpawn({ some: 'bad' }) }),
