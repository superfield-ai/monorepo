...
beforeEach(async () => {
  tmpRepo = await fs.mkdtemp(path.join(os.tmpdir(), 'superfield-docloop-test-'));
  await fs.mkdir(path.join(tmpRepo, 'docs'), { recursive: true });
  await fs.writeFile(path.join(tmpRepo, 'docs/prd.md'), '# PRD\n## Commands\nold text\n');
...
function makeClient(overrides: Partial<GitHubClient> = {}): GitHubClient {
  return {
    ...
    createPullRequest: vi.fn().mockResolvedValue({ number: 999, html_url: 'url' }),
    ...overrides,
  } as unknown as GitHubClient;
}

/** Returns a spawn that emits a different JSON for each consecutive call. */
function multiSpawn(responses: unknown[]): (opts: AgentOpts) => Promise<AgentResult> {
  let idx = 0;
  return async (_opts) => ({
    sessionId: `s-${idx}`,
...
describe('tickDocLoop', () => {
  ...
  it('returns idle when newest merged PR is older than watermark', async () => {
    const client = makeClient({
      listMergedPullRequests: vi.fn().mockResolvedValue([makePR({ merged_at: '2026-04-07T00:00:00Z' })]),
    });
    const result = await tickDocLoop({
      client,
      owner: 'o',
  ...
  it('opens a doc PR when canonical sync produces patches', async () => {
    ...
    const spawn = multiSpawn([
      { missing_docs: [] },
      {
        ...
        prd_patches: [{ section: '## Commands', old_text: 'old text', new_text: 'new text' }],
        readme_patches: [],
      },
      { inconsistencies: [] },
    ]);
    ...
    const putCall = (client.putFileContents as ReturnType<typeof vi.fn>).mock.calls[0]![0];
  ...
  it('skips coverage and consistency tasks when no source files in the PR', async () => {
    const client = makeClient({
      listMergedPullRequests: vi.fn().mockResolvedValue([makePR()]),
      listPullRequestFiles: vi.fn().mockResolvedValue(['docs/prd.md', 'README.md']),
    });
    const spawn = vi.fn().mockImplementation(multiSpawn([
      { significant: false, prd_patches: [], readme_patches: [] },
    ]));
  ...
  it('filters out test files from source-file analysis', async () => {
    ...
    const spawn = vi.fn().mockImplementation(async (opts: AgentOpts) => {
      ...
      if (opts.prompt.includes('doc-coverage')) {
        return { sessionId: 's', output: JSON.stringify({ missing_docs: [] }), isError: false };
      }
      if (opts.prompt.includes('doc-canonical-sync')) {
        return { sessionId: 's', output: JSON.stringify({ significant: false, prd_patches: [], readme_patches: [] }), isError: false };
      }
      return { sessionId: 's', output: JSON.stringify({ inconsistencies: [] }), isError: false };
    });
    await tickDocLoop({
      client,
      owner: 'o',
