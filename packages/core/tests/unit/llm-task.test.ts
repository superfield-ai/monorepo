...
describe("extractJson — strict mode", () => {
  ...
  it("ignores braces inside strings", () => {
    expect(extractJson('{"text":"hello { world }"}')).toBe(
      '{"text":"hello { world }"}',
    );
  });
  ...
  it("refuses inline JSON wrapped in prose (no fence, leading text)", () => {
    expect(extractJson('The answer is {"answer": 42} as requested.')).toBe(
      null,
    );
  });

  it("refuses inline JSON when prose has braces before the real object", () => {
    expect(extractJson('Result is {something in braces}: {"actual":1}')).toBe(
      null,
    );
  });
...
describe("runLLMTask", () => {
  ...
  it("throws when agent reports error", async () => {
    await expect(
      runLLMTask({ prompt: "x", spawn: fakeSpawn("boom", true) }, (json) =>
        JSON.parse(json),
      ),
  ...
  it("throws when response contains no JSON", async () => {
    await expect(
      runLLMTask({ prompt: "x", spawn: fakeSpawn("just prose") }, (json) =>
        JSON.parse(json),
      ),
  ...
  it("throws when parse function throws", async () => {
    await expect(
      runLLMTask({ prompt: "x", spawn: fakeSpawn('{"a":1}') }, () => {
