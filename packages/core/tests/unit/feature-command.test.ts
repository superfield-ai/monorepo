...
describe('runFeatureCommand', () => {
  it('creates issue and Plan when both are absent', async () => {
    ...
    const featureCall = (client.createIssue as ReturnType<typeof vi.fn>).mock.calls.find(
      (c) => c[0].labels?.includes('feature'),
    );
...
`;
    const client = makeClient({
      listIssues: vi.fn().mockResolvedValue([
        { number: 99, body: planBody, labels: ['plan'], state: 'open', html_url: '', title: 'Plan' },
    ...
    const updateBody = (client.updateIssueBody as ReturnType<typeof vi.fn>).mock.calls[0]![0].body;
  ...
  it('passes open issue titles to the prompt for duplicate detection', async () => {
    ...
    await runFeatureCommand({
      ...
      spawn: async (opts: AgentOpts) => {
        ...
        return { sessionId: 's', output: JSON.stringify(validEvaluation), isError: false };
      },
    });
