...
describe('upsertSession', () => {
  it('creates a new comment when none exists', async () => {
    ...
    const args = (client.createIssueComment as ReturnType<typeof vi.fn>).mock.calls[0]!;
  ...
  it('updates an existing comment when one exists', async () => {
    ...
    await upsertSession(client, 'o', 'r', 10, sampleSession);
    expect(client.updateIssueComment).toHaveBeenCalledWith('o', 'r', 99, expect.any(String));
  ...
  it('returns sessions older than the timeout', async () => {
    ...
    const client = makeClient({
      ...
      listIssueComments: vi
        .fn()
        .mockResolvedValueOnce([{ id: 1, body: sessionCommentBody(oldSession) }])
        .mockResolvedValueOnce([
          { id: 2, body: sessionCommentBody({ ...sampleSession, startedAt: new Date().toISOString() }) },
        ]),
    });
  ...
  it('returns empty when all sessions are fresh', async () => {
    const client = makeClient({
      ...
      listIssueComments: vi
        .fn()
        .mockResolvedValue([
          { id: 1, body: sessionCommentBody({ ...sampleSession, startedAt: new Date().toISOString() }) },
        ]),
    });
    const stale = await findStaleSessions(client, 'o', 'r', 30_000);
