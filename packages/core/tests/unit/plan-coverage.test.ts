...
describe('runPlanCoverage', () => {
  it('creates the Plan issue on first run when open issues exist', async () => {
    ...
    const createArg = (client.createIssue as ReturnType<typeof vi.fn>).mock.calls[0]![0];
...
`;
    const planIssue = { number: 99, title: 'Plan', body: existingPlanBody, labels: ['plan'], state: 'open', html_url: '' };
    const openIssues = [
    ...
    const body = (client.updateIssueBody as ReturnType<typeof vi.fn>).mock.calls[0]![0].body;
  ...
  it('skips ci-failure and plan-labelled issues', async () => {
    const openIssues = [
      ...
      makeIssue({ number: 20, title: 'fix: ci failure', labels: ['ci-failure'] }),
  ...
  it('classifies dev-scout labelled issues with kind dev-scout', async () => {
    ...
    const body = (client.createIssue as ReturnType<typeof vi.fn>).mock.calls[0]![0].body;
