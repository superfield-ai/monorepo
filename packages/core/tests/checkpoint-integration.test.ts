...
function git(args: string, cwd?: string): string {
  return execSync(`git ${args}`, { cwd: cwd ?? repoDir, encoding: 'utf8' }).trim();
}
...
describe('checkpoint integration — full cycle', () => {
  it('creates a checkpoint after a simulated edit and shows it in the timeline', () => {
    ...
    // Verify the timeline
    const timeline = getTimeline({ worktreePath: repoDir, baseRef: initialHash });
...
describe('checkpoint integration — rollback', () => {
  it('creates three checkpoints and rolls back to the first, leaving only one on the branch', () => {
    ...
    const cp1 = createCheckpoint({ worktreePath: repoDir, summary: 'Added file one' });
    expect(cp1.created).toBe(true);

    // Checkpoint 2
    writeFileSync(join(repoDir, 'file2.ts'), 'export const b = 2;\n');
    const cp2 = createCheckpoint({ worktreePath: repoDir, summary: 'Added file two' });
    expect(cp2.created).toBe(true);

    // Checkpoint 3
    writeFileSync(join(repoDir, 'file3.ts'), 'export const c = 3;\n');
    const cp3 = createCheckpoint({ worktreePath: repoDir, summary: 'Added file three' });
    ...
    expect(readFileSync(join(repoDir, 'file1.ts'), 'utf8')).toBe('export const a = 1;\n');
  });
  ...
  it('rolling back updates the timeline to reflect the new HEAD', () => {
    ...
    const cp1 = createCheckpoint({ worktreePath: repoDir, summary: 'First change' });
...
describe('checkpoint integration — timeline is linear', () => {
  it('timeline contains no branches or forks', () => {
    ...
    for (let i = 1; i <= 5; i++) {
      ...
      createCheckpoint({ worktreePath: repoDir, summary: `Change number ${i}` });
    }
