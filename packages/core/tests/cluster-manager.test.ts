...
describe('applyManifests', () => {
  ...
  it('rewrites ghcr.io image tags to studio tag', async () => {
    vi.mocked(readFileSync).mockReturnValue(
      'spec:\n  template:\n    spec:\n      containers:\n        - image: ghcr.io/myorg/calypso-starter-ts:latest\n',
    );

    const { applyManifests } = await import('../cluster-manager');

    applyManifests(config, '/product/k8s', {
      'ghcr.io/myorg/calypso-starter-ts:latest': 'calypso-release:studio',
    });
    ...
    expect(piped).toContain('calypso-release:studio');
    expect(piped).not.toContain('ghcr.io/myorg/calypso-starter-ts');
  });

  it('rewrites <owner> angle-bracket placeholder image tags', async () => {
    vi.mocked(readFileSync).mockReturnValue(
      'spec:\n  template:\n    spec:\n      containers:\n        - image: ghcr.io/<owner>/calypso-starter-ts:latest\n',
    );

    const { applyManifests } = await import('../cluster-manager');

    applyManifests(config, '/product/k8s', {
      'ghcr.io/<owner>/calypso-starter-ts:latest': 'calypso-release:studio',
    });
