...
function makeClient(overrides: Partial<GitHubClient> = {}): GitHubClient {
  return {
    ...
    listIssues: vi.fn().mockResolvedValue([
      makeIssue({ number: 99, labels: ['plan'], body: conformantPlanBody, title: 'Plan' }),
      makeIssue({ number: 1 }),
    ]),
    getIssue: vi.fn().mockResolvedValue(makeIssue()),
    listIssueComments: vi.fn().mockResolvedValue([]),
    createIssue: vi.fn().mockResolvedValue({ number: 100, title: 'CI failure', labels: [], body: '', html_url: '' }),
    ...
    const client = makeClient({
      ...
      listIssues: vi.fn(async (_o, _r, labels?: string[]) => {
        if (labels?.includes('plan')) {
          return [makeIssue({ number: 99, title: 'Plan', labels: ['plan'], body: conformantPlanBody })];
        }
        return [makeIssue()];
