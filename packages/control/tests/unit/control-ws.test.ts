...
describe('turn frame', () => {
  it('default mode is design (not passed in turn frame)', async () => {
    ...
    await controlWsHandler.message(ws, JSON.stringify({ type: 'turn', message: 'hello' }));
  ...
  it('mode: question is forwarded to streamTurn', async () => {
    ...
    const ws = makeWs({ _streamTurn: streamTurnSpy });
    await controlWsHandler.message(ws, JSON.stringify({ type: 'turn', message: 'q', mode: 'question' }));
  ...
  it('SSE data line → chunk frame', async () => {
    ...
    await controlWsHandler.message(ws, JSON.stringify({ type: 'turn', message: 'test' }));
  ...
  it('second turn frame aborts the first AbortController', async () => {
    ...
    const p1 = controlWsHandler.message(ws, JSON.stringify({ type: 'turn', message: 'first' }));
    ...
    await controlWsHandler.message(ws, JSON.stringify({ type: 'turn', message: 'second' }));
...
describe('steer frame', () => {
  it('successful POST → steer-ack with requestId', async () => {
    ...
    await controlWsHandler.message(ws, JSON.stringify({ type: 'steer', context: 'focus on auth' }));
  ...
  it('fetch throws → error frame with "Steer failed:" prefix', async () => {
    ...
    await controlWsHandler.message(ws, JSON.stringify({ type: 'steer', context: 'x' }));
  ...
  it('POST goes to <superfieldApiUrl>/steer/context', async () => {
    ...
    await controlWsHandler.message(ws, JSON.stringify({ type: 'steer', context: 'focus' }));
