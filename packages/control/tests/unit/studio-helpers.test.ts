...
describe('parseControlInfo', () => {
  it('parses valid studio session JSON', () => {
    expect(parseControlInfo('{"sessionId":"a1b2","branch":"studio/session-main-a1b2"}')).toEqual({
      sessionId: 'a1b2',
  ...
  it('returns null when required fields are missing', () => {
    ...
    expect(parseControlInfo('{"branch":"studio/session-main-a1b2"}')).toBeNull();
  });
});
...
describe('request validation helpers', () => {
  it('accepts a non-empty message and trims surrounding whitespace', () => {
    expect(validateControlMessage('  update the header  ')).toBe('update the header');
  });
