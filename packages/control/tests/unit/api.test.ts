 ...
 *   - SUPERFIELD_REPO_ROOT points to a temp directory so real fs calls hit
 ...
 *   - api.test.ts asserts that handleControlRequest returns 401 for unauthenticated requests
...
//
// Instead of mocking fs, we set SUPERFIELD_REPO_ROOT to a temp directory so that
...
afterAll(() => {
  if (originalRepoRoot !== undefined) {
    process.env.SUPERFIELD_REPO_ROOT = originalRepoRoot;
  } else {
...
describe("handleControlRequest — negative paths", () => {
  ...
  it("returns 403 when studio mode is not active for authenticated GET /studio/commits", async () => {
    ...
    // it hits the isControlMode() guard which returns 403.
