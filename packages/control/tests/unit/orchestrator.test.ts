...
function makeFakeLoop(
  state = "stopped" as ReturnType<DevLoopProcess["status"]>,
  apiReachable = false,
) {
  const logs: string[] = [];
  return {
...
describe("non-matching path", () => {
  it("/something-else → returns null", async () => {
    ...
    const res = await handleOrchestratorRequest(
      req,
      makeUrl("/something-else"),
      API_URL,
    );
    expect(res).toBeNull();
  });
...
describe("POST /orchestrator/start", () => {
  ...
  it("valid repo → 200 ok: true", async () => {
    ...
    expect(loop.spawn).toHaveBeenCalledWith("/my/repo", {
      slotCount: undefined,
    });
  });
  ...
  it("slotCount forwarded to spawn", async () => {
    ...
    await handleOrchestratorRequest(
      req,
      makeUrl("/orchestrator/start"),
      API_URL,
    );
