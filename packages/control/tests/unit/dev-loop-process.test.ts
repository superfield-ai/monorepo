...
import {
  DevLoopProcess,
  type DevLoopProcessOpts,
} from "../../src/dev-loop-process";
...
function makeOpts(
  overrides: Partial<DevLoopProcessOpts> = {},
): DevLoopProcessOpts {
  return {
...
describe("onStateChange", () => {
  it("NOT called when transitioning to the same state", async () => {
    const calls: string[] = [];
    const loop = new DevLoopProcess(
      makeOpts({ onStateChange: (s) => calls.push(s) }),
    );
