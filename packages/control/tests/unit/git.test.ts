...
describe('parseSessionCommits', () => {
  ...
  it('parses multiple --oneline entries', () => {
    ...
    expect(commits[0]).toEqual({ hash: 'deadbeef', message: 'refactor: clean up router' });
    expect(commits[1]).toEqual({ hash: 'cafebabe', message: 'fix: correct cookie path' });
    expect(commits[2]).toEqual({ hash: 'f00d1234', message: 'docs: update README' });
  });
  ...
  it('handles a commit message that contains spaces', () => {
    ...
    const commits = parseSessionCommits(output);
    expect(commits[0].message).toBe('feat(auth): add JWT sign and verify helpers');
  });
