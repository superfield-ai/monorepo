...
const STUDIO_DOWN_SCRIPT = join(STUDIO_ROOT, "scripts", "studio-down.ts");

// Skip when scripts/studio-down.ts is absent (cli monorepo, not standalone studio repo).
describe.skipIf(!existsSync(STUDIO_DOWN_SCRIPT))("studio-scripts suite", () => {
  // ── Helpers ───────────────────────────────────────────────────────────────────
