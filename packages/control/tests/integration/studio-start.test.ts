...
// The "studio" script only exists in the standalone studio repo, not the cli monorepo.
const STUDIO_SCRIPT_EXISTS = (() => {
  try {
    const pkg = JSON.parse(
      readFileSync(join(REPO_ROOT, "package.json"), "utf8"),
    ) as { scripts?: Record<string, string> };
    return "studio" in (pkg.scripts ?? {});
  } catch {
    return false;
  }
})();

describe.skipIf(!STUDIO_SCRIPT_EXISTS)("studio-start suite", () => {
  const CLONE_ROOT = join("/tmp", `superfield-studio-test-${Date.now()}`);
  ...
  function resolveStudioMainHash(cwd: string): string {
    ...
    throw new Error(
      "Could not resolve a main branch ref for the Studio test clone.",
    );
  }
  ...
  test("bun run studio bootstraps an existing studio branch checkout and exits after bootstrap in test mode", async () => {
    ...
    spawnGitSync(
      ["config", "user.email", "studio-start-test@example.com"],
      CLONE_ROOT,
    );
    ...
    const checkedOutBranchName = (branch.stdout ?? new Uint8Array())
      .toString()
      .trim();
    ...
    expect(readFileSync(changesPath, "utf8")).toContain(
      `# Studio Session — ${branchName}`,
    );
    ...
    do {
      containerInspect = Bun.spawnSync(
        ["docker", "inspect", studioInfo.containerId],
        {
          cwd: CLONE_ROOT,
          stdout: "pipe",
          stderr: "pipe",
