...
import { startSuperfieldFixture, type SuperfieldFixture } from './helpers/superfield-server';
...
describe('GET /studio/chat/stream — SSE streaming via API fixture', () => {
  ...
  it('returns Content-Type: text/event-stream for a valid message', async () => {
    const config = makeConfig({ logDir: tmpDir, superfieldApiUrl: fixture.apiUrl });
    const req = new Request('http://localhost:7000/studio/chat/stream?message=hello+world');
    const res = await route(req, config);
  ...
  it('streams Claude stdout as SSE data events and ends with a done event', async () => {
    const config = makeConfig({ logDir: tmpDir, superfieldApiUrl: fixture.apiUrl });
    const req = new Request('http://localhost:7000/studio/chat/stream?message=hello+from+test');
    const res = await route(req, config);
    expect(res.status).toBe(200);
  ...
  it('writes a JSONL log entry after the turn completes', async () => {
    const config = makeConfig({ logDir: tmpDir, superfieldApiUrl: fixture.apiUrl });
    const req = new Request('http://localhost:7000/studio/chat/stream?message=log+test+message');
    const res = await route(req, config);
    await res.text();
