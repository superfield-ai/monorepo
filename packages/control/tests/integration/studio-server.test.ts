 ...
 */
async function startRouterServer(config: ControlConfig): Promise<{ baseUrl: string; server: Server }> {
...
async function startFakeServer(
  handler: (req: IncomingMessage) => { status: number; headers?: Record<string, string>; body: string },
...
describe('studio server — proxy to web service', () => {
  ...
  beforeAll(async () => {
    ...
    ({ baseUrl: studioBaseUrl, server: routerServer } = await startRouterServer(config));
  });
