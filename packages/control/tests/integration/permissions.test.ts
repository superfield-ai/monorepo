...
describe('Studio session — permission sandbox enforcement', () => {
  ...
  it('Claude cannot make outbound HTTP requests', () => {
    ...
    expect(validateBashCommand('wget https://example.com/file.tar.gz').allowed).toBe(false);
  ...
  it('forbidden action attempt produces a clear, non-silent denial', () => {
    const msg = buildPermissionDeniedMessage('Bash', 'Bash is blocked in studio mode');
  ...
  it('Bash command validation catches git in complex command chains', () => {
    ...
    // In a pipeline
    expect(validateBashCommand('cat file | git hash-object --stdin').allowed).toBe(false);
    // With env vars
