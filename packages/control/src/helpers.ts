 ...
 * ## Exports
 *
 *   - ControlInfo / ControlMessage / ControlMode — shared data types
 *   - parseControlInfo() — parse the .studio session file
 *   - parseSessionCommits() / parseTimelineCommits() — parse git log output
 *   - validateControlMessage() / validateRollbackHash() — input validation
 ...
 *
 * @returns Parsed ControlInfo, or null if the file is missing or malformed.
 */
...
export function getStudioSystemPrompt(branch: string): string {
  return `You are a studio mode agent for Superfield. You are helping a business partner explore UI and workflow changes to the Superfield application in a live session.
