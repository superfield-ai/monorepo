...
describe("DebugView", () => {
  ...
  test("filters by level, source, and search text", async () => {
    ...
    await expect.element(screen.getByTestId("debug-view-count")).toHaveTextContent(
      "3 / 3",
    );
    await expect.element(screen.getByText("Alpha console warning")).toBeVisible();
    await expect.element(screen.getByText("Beta fetch request")).toBeVisible();
    await expect.element(screen.getByText("Gamma backend failure")).toBeVisible();

    await screen.getByTestId("debug-filter-level-warn").click();
    await expect.element(screen.getByTestId("debug-view-count")).toHaveTextContent(
      "2 / 3",
    );
    expect(screen.container.textContent ?? "").not.toContain("Alpha console warning");
    await expect.element(screen.getByText("Beta fetch request")).toBeVisible();
    await expect.element(screen.getByText("Gamma backend failure")).toBeVisible();
    ...
    expect(screen.container.textContent ?? "").not.toContain("Beta fetch request");
    await expect.element(screen.getByText("Gamma backend failure")).toBeVisible();
  ...
  test("expanding a row reveals context, stack, breadcrumbs, and action buttons", async () => {
    ...
    await screen.getByRole("button", { name: /gamma backend failure/i }).click();
    ...
    await expect.element(screen.getByText(/\[route\]\s+→\s+\/studio/)).toBeVisible();
  ...
  test("copy to clipboard writes the entry and shows a success toast", async () => {
    ...
    expect(writeText.mock.calls[0]?.[0]).toContain('"message": "Clipboard test entry"');
    expect(toastStore.getAll().some((t) => t.title === "COPIED — DEBUG ENTRY")).toBe(
      true,
    );
  });
