...
describe('GitHubClient.listAllInstallations', () => {
  it('returns empty array when no installations exist', async () => {
    server.use(
      http.get(`${BASE}/user/installations`, () => HttpResponse.json(installationsEmpty)),
    );
  ...
  it('returns personal account installation with selected repos', async () => {
    server.use(
      http.get(`${BASE}/user/installations`, () => HttpResponse.json(installationsPersonalSelected)),
    );
  ...
  it('returns org installation with selected repos', async () => {
    server.use(
      http.get(`${BASE}/user/installations`, () => HttpResponse.json(installationsOrgSelected)),
    );
  ...
  it('returns installation with all-repos selection', async () => {
    server.use(
      http.get(`${BASE}/user/installations`, () => HttpResponse.json(installationsAllRepos)),
    );
...
describe('GitHubClient.listAppInstallations', () => {
  it('filters by app slug case-insensitively', async () => {
    const mixed = {
      total_count: 2,
      installations: [
        { ...installationsOrgSelected.installations[0], app_slug: 'Superfield-CLI' },
        { ...installationsPersonalSelected.installations[0], app_slug: 'other-app', id: 99 },
      ],
    };
    server.use(
