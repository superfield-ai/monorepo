...
describe("pollGitHubAppAccessToken", () => {
  ...
  it("keeps polling until the token is granted", async () => {
    const fetchImpl = vi
      ...
      .mockResolvedValueOnce(
        new Response("error=authorization_pending", { status: 200 }),
      )
      .mockResolvedValueOnce(
        new Response(
  ...
  it("accepts token responses without a scope field", async () => {
    const fetchImpl = vi.fn().mockResolvedValue(
      new Response("access_token=token-123&token_type=bearer", {
        status: 200,
      }),
    );
  ...
  it("throws on access denied", async () => {
    const fetchImpl = vi
      .fn()
      .mockResolvedValue(new Response("error=access_denied", { status: 200 }));
