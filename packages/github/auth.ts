...
export interface DeviceFlowError extends Error {
  name: 'authorization_pending' | 'slow_down' | 'expired_token' | 'access_denied' | 'device_flow_disabled' | 'incorrect_client_credentials' | 'incorrect_device_code' | 'unsupported_grant_type' | 'unknown_error';
}
...
export async function pollGitHubAppAccessToken(
  ...
  while (true) {
    ...
    url.searchParams.set('grant_type', 'urn:ietf:params:oauth:grant-type:device_code');
...
async function parseFormResponse(response: Response): Promise<Record<string, string>> {
...
function optionalString(data: Record<string, string>, key: string): string | undefined {
  return data[key] || undefined;
}

function optionalNumber(data: Record<string, string>, key: string): number | undefined {
  const value = data[key];
  if (!value) {
    return undefined;
  }
...
function deviceFlowError(name: string): DeviceFlowError {
  const error = new Error(`GitHub device flow failed: ${name}`) as DeviceFlowError;
  error.name =
