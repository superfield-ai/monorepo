...
import { studioWsHandler, type WsData } from '../../src/studio-ws';
...
describe('invalid JSON', () => {
  it('sends error frame', async () => {
    const ws = makeWs();
    await studioWsHandler.message(ws, 'not-json');
    expect(ws.frames()[0]).toEqual({ type: 'error', message: 'Invalid JSON frame' });
  });
});
...
describe('turn frame', () => {
  it('default mode is design (not passed in turn frame)', async () => {
    const streamTurnSpy = vi.fn().mockReturnValue(makeSseStream(
      'event: session\ndata: {"sessionId":"sid1"}\n\nevent: done\ndata: {"filesChanged":[]}\n\n',
    ));
    const ws = makeWs({ _streamTurn: streamTurnSpy });
    await studioWsHandler.message(ws, JSON.stringify({ type: 'turn', message: 'hello' }));
    const [, , , mode] = streamTurnSpy.mock.calls[0] as [string, string, string | undefined, string];
  ...
  it('mode: question is forwarded to streamTurn', async () => {
    ...
    const ws = makeWs({ _streamTurn: streamTurnSpy });
    await studioWsHandler.message(ws, JSON.stringify({ type: 'turn', message: 'q', mode: 'question' }));
    const [, , , mode] = streamTurnSpy.mock.calls[0] as unknown as [string, string, string, string];
  ...
  it('SSE data line → chunk frame', async () => {
    ...
    await studioWsHandler.message(ws, JSON.stringify({ type: 'turn', message: 'test' }));
    const chunks = ws.frames().filter((f) => f.type === 'chunk');
    expect(chunks.length).toBeGreaterThan(0);
    expect(chunks.some((c) => (c as { text: string }).text.includes('hello world'))).toBe(true);
  });
  ...
  it('event:session captures sessionId, included in final done frame', async () => {
    ...
    await studioWsHandler.message(ws, JSON.stringify({ type: 'turn', message: 'test' }));
    const done = ws.frames().find((f) => f.type === 'done') as { sessionId: string; filesChanged: string[] } | undefined;
  ...
  it('reader throws non-AbortError → error frame', async () => {
    const errorStream = new ReadableStream<Uint8Array>({
      start(c) { c.error(new Error('network blew up')); },
    });
  ...
  it('second turn frame aborts the first AbortController', async () => {
    ...
    const ws = makeWs({
      _streamTurn: (_msg, _key, _dir, _mode, fetchFn) => {
        ...
        if (calls === 1) {
          ...
          const neverClosing = new ReadableStream<Uint8Array>({ start() { /* never resolves */ } });
    ...
    const p1 = studioWsHandler.message(ws, JSON.stringify({ type: 'turn', message: 'first' }));
    ...
    await studioWsHandler.message(ws, JSON.stringify({ type: 'turn', message: 'second' }));
...
describe('steer frame', () => {
  it('successful POST → steer-ack with requestId', async () => {
    ...
    await studioWsHandler.message(ws, JSON.stringify({ type: 'steer', context: 'focus on auth' }));
    const ack = ws.frames().find((f) => f.type === 'steer-ack') as { requestId: string } | undefined;
  ...
  it('fetch throws → error frame with "Steer failed:" prefix', async () => {
    ...
    await studioWsHandler.message(ws, JSON.stringify({ type: 'steer', context: 'x' }));
    const err = ws.frames().find((f) => f.type === 'error') as { message: string } | undefined;
  ...
  it('POST goes to <superfieldApiUrl>/steer/context', async () => {
    ...
    const ws = makeWs({ superfieldApiUrl: 'http://custom:9999', _fetch: fetchSpy });
    await studioWsHandler.message(ws, JSON.stringify({ type: 'steer', context: 'focus' }));
...
describe('close', () => {
  it('aborts in-flight AbortController on disconnect', () => {
    ...
    studioWsHandler.close(ws);
    expect(ac.signal.aborted).toBe(true);
  });

  it('no-op when no AbortController present', () => {
    const ws = makeWs();
    expect(() => studioWsHandler.close(ws)).not.toThrow();
  });
});
