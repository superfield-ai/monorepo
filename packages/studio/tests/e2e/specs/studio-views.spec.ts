 ...
 *
 * Run:
 *   npx playwright test --config packages/studio/tests/e2e/playwright.config.ts
 */
...
test.describe('tab bar', () => {
  ...
  test('Studio tab is active by default', async ({ page }) => {
    ...
    // Other tabs do not.
    await expect(page.getByTestId('tab-orchestrator')).not.toHaveClass(/border-b-2/);
...
test.describe('Orchestrator tab', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="process-state-badge"]', { timeout: 10_000 });
  });
  ...
  test('Start button is present and disabled without a repo path', async ({ page }) => {
...
test.describe('Preview tab', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="preview-view-selector"]', { timeout: 10_000 });
  });

  test('Studio and Orchestrator tab content disappears after switching', async ({ page }) => {
    await expect(page.getByTestId('chat-panel')).not.toBeVisible();
  ...
  test('WikiRender content shows a heading', async ({ page }) => {
    ...
    await expect(
      page.getByTestId('preview-content').getByRole('heading', { name: 'Component Preview' }),
    ).toBeVisible();
  });

  test('WikiRender output escapes HTML — no live <script> in DOM', async ({ page }) => {
  ...
  test('clicking Citations shows the citation marker', async ({ page }) => {
    ...
    await expect(page.getByTestId('preview-content').getByText('[1]')).toBeVisible();
  });
...
test.describe('tab switching', () => {
  test('switching back to Studio tab restores the chat panel', async ({ page }) => {
    await goToStudio(page);
    await page.getByTestId('tab-orchestrator').click();
