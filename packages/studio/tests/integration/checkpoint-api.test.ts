...
const CLONE_ROOT = join('/tmp', `calypso-checkpoint-api-${Date.now()}`);
...
beforeAll(async () => {
  ...
  spawnSync('git', ['config', 'user.email', 'checkpoint-api-test@example.com'], {
    cwd: CLONE_ROOT,
    stdio: 'pipe',
  });
...
test(
  'POST /studio/chat with a Design-mode message that produces file changes returns a timeline entry',
  async () => {
    ...
    const chatRes = await fetch(`${BASE}/studio/chat`, {
      ...
      body: JSON.stringify({ message: 'Please update the header colour to blue.' }),
    });
...
test(
  'GET /studio/timeline returns the same checkpoint entry that chat produced',
  async () => {
...
test(
  'POST /studio/rollback with a checkpoint hash removes it from GET /studio/timeline',
  async () => {
