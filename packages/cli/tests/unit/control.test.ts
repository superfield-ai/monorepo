...
describe('parseControlArgs', () => {
  it('empty args → all fields absent, help false', () => {
    ...
    expect(r).toEqual({ port: undefined, repo: undefined, apiUrl: undefined, help: false, unknown: [] });
  });
  ...
  it('--repo /path/to/repo → repo set', () => {
    expect(parseControlArgs(['--repo', '/path/to/repo']).repo).toBe('/path/to/repo');
  });
  ...
  it('--api-url http://localhost:7837 → apiUrl set', () => {
    expect(parseControlArgs(['--api-url', 'http://localhost:7837']).apiUrl).toBe('http://localhost:7837');
  });
  ...
  it('multiple unknown flags → all accumulated', () => {
    expect(parseControlArgs(['--verbose', '--debug']).unknown).toEqual(['--verbose', '--debug']);
  });

  it('all three flags together', () => {
    const r = parseControlArgs(['--port', '8000', '--repo', '/app', '--api-url', 'http://x:7837']);
    expect(r).toMatchObject({ port: 8000, repo: '/app', apiUrl: 'http://x:7837', help: false, unknown: [] });
  });
});

// ── controlCommand ────────────────────────────────────────────────────────────

function makeDeps(overrides: Partial<ControlCommandDeps> = {}): ControlCommandDeps {
  return {
    log: vi.fn(),
    warn: vi.fn(),
...
describe('controlCommand', () => {
  ...
  it('--help output contains option docs', async () => {
    let output = '';
    const deps = makeDeps({ log: (m) => { output += m; } });
    await controlCommand(['--help'], deps);
  ...
  it('health check non-200 → warns with HTTP status', async () => {
    const deps = makeDeps({
      _fetch: vi.fn().mockResolvedValue({ ok: false, status: 503 } as Response),
    });
    await controlCommand([], deps);
