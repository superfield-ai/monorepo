...
describe("runGithubAdd", () => {
  ...
  it("runs device flow when no user is configured", async () => {
    ...
    const deps = makeDeps({
      ...
      pollAccessToken: vi
        .fn()
        .mockResolvedValue({ accessToken: "ghu_new", tokenType: "bearer" }),
      fetchUserLogin: vi.fn().mockResolvedValue("octocat"),
      saveConfig: vi.fn().mockResolvedValue(undefined),
      resolveRepo: vi.fn().mockResolvedValue({ owner: "org", repo: "repo" }),
    });
  ...
  it("runs device flow when existing token is expired", async () => {
    ...
    const deps = makeDeps({
      ...
      pollAccessToken: vi
        .fn()
        .mockResolvedValue({ accessToken: "ghu_fresh", tokenType: "bearer" }),
      saveConfig: vi.fn().mockResolvedValue(undefined),
      resolveRepo: vi.fn().mockResolvedValue({ owner: "org", repo: "repo" }),
    });
    ...
    expect(deps.requestDeviceCode).toHaveBeenCalledWith(
      DEFAULT_GITHUB_APP_CLIENT_ID,
    );
...
describe("runGithubForget", () => {
  ...
  it("does nothing when no account is configured", async () => {
    ...
    expect(deps.log).toHaveBeenCalledWith(
      "Nothing to forget — no GitHub account configured.",
    );
  });
