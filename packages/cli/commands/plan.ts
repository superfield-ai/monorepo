...
export async function planCommand(repoPath?: string): Promise<void> {
  ...
  const result = await runPlanCommand({ client, owner, repo, cwd: resolvedPath });

  if (result.scoutsCreated.length > 0) {
    console.log(`✓ Created ${result.scoutsCreated.length} scout issue(s): ${result.scoutsCreated.map((n) => `#${n}`).join(', ')}`);
  }
