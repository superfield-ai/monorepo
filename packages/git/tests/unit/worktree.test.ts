...
beforeEach(async () => {
  tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'superfield-worktree-test-'));
});
...
describe('WorktreeManager.worktreePath', () => {
  it('builds a deterministic path from owner, repo, issue, slug', () => {
    ...
    const p = wm.worktreePath('dot-matrix-labs', 'superfield-ts', 42, 'fix-loop-bug');
    expect(p).toBe('/tmp/test/dot-matrix-labs__superfield-ts/issue-42-fix-loop-bug');
  });
...
describe('WorktreeManager.list', () => {
  ...
  it('lists worktree directories matching the issue-N-slug pattern', async () => {
    const wm = new WorktreeManager({ root: tmpRoot });
    await fs.mkdir(path.join(tmpRoot, 'org__repo', 'issue-10-foo'), { recursive: true });
    await fs.mkdir(path.join(tmpRoot, 'org__repo', 'issue-20-bar-baz'), { recursive: true });
    // Non-conforming directory should be ignored
    await fs.mkdir(path.join(tmpRoot, 'org__repo', 'random-dir'), { recursive: true });
...
describe('WorktreeManager.pruneClosed', () => {
  it('removes only worktrees whose issue numbers are in the closed set', async () => {
    const wm = new WorktreeManager({ root: tmpRoot });
    await fs.mkdir(path.join(tmpRoot, 'org__repo', 'issue-10-foo'), { recursive: true });
    await fs.mkdir(path.join(tmpRoot, 'org__repo', 'issue-20-bar'), { recursive: true });
    await fs.mkdir(path.join(tmpRoot, 'org__repo', 'issue-30-baz'), { recursive: true });
