...
describe('parseGitHubRemote', () => {
  it('parses https remote with .git suffix', () => {
    const r = parseGitHubRemote('https://github.com/dot-matrix-labs/superfield-ts.git');
    expect(r).toEqual({ owner: 'dot-matrix-labs', repo: 'superfield-ts' });
  });

  it('parses https remote without .git suffix', () => {
    const r = parseGitHubRemote('https://github.com/dot-matrix-labs/superfield-ts');
    expect(r).toEqual({ owner: 'dot-matrix-labs', repo: 'superfield-ts' });
  });

  it('parses ssh remote', () => {
    const r = parseGitHubRemote('git@github.com:dot-matrix-labs/superfield-ts.git');
    expect(r).toEqual({ owner: 'dot-matrix-labs', repo: 'superfield-ts' });
  });
