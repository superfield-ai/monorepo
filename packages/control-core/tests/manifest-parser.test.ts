...
describe("discoverResources", () => {
  it("discovers all workloads from the product k8s directory", () => {
    ...
    expect(names).toContain("superfield-app");
    expect(names).toContain("superfield-postgres");
    expect(names).toContain("superfield-db-init");
  });

  it("discovers worker deployments", () => {
    const resources = discoverResources(K8S_DIR);
    const names = resources.map((r) => r.name);
    expect(names).toContain("superfield-worker-coding");
    expect(names).toContain("superfield-worker-analysis");
  });
...
describe("discoverSecretRefs", () => {
  it("finds the superfield-secrets secret", () => {
    const secrets = discoverSecretRefs(K8S_DIR);
    const names = secrets.map((s) => s.name);
    expect(names).toContain("superfield-secrets");
  });

  it("finds expected keys in superfield-secrets", () => {
    const secrets = discoverSecretRefs(K8S_DIR);
    const superfieldSecrets = secrets.find(
      (s) => s.name === "superfield-secrets",
    );
    expect(superfieldSecrets).toBeDefined();
    expect(Object.keys(superfieldSecrets!.literals)).toContain("DATABASE_URL");
    expect(Object.keys(superfieldSecrets!.literals)).toContain(
      "POSTGRES_PASSWORD",
    );
  });
  ...
  it("finds worker secrets", () => {
    const secrets = discoverSecretRefs(K8S_DIR);
    const names = secrets.map((s) => s.name);
    expect(names).toContain("superfield-worker-coding-secret");
    expect(names).toContain("superfield-worker-analysis-secret");
  });

  it("finds db-init secret", () => {
    const secrets = discoverSecretRefs(K8S_DIR);
    const names = secrets.map((s) => s.name);
    expect(names).toContain("superfield-db-init-secret");
  });
...
describe("discoverServicePort", () => {
  ...
  // repo that represents a superfield app's k8s directory.
  const FIXTURE_K8S_DIR = resolve(
    dirname(fileURLToPath(import.meta.url)),
    "..",
    "..",
    "..",
...
describe("discoverImages", () => {
  ...
  it("does not include commented-out images", () => {
    ...
    const appImages = images.filter((i) => i.includes("superfield-starter-ts"));
