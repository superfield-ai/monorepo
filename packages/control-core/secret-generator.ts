    ...
    literals: Object.fromEntries(
      Object.keys(spec.literals).map((key) => {
        ...
        if (k.includes("DATABASE_URL")) {
          ...
          const db = k.includes("AUDIT")
            ? "superfield_audit"
            : k.includes("ANALYTICS")
              ? "superfield_analytics"
              : pgDb;
          return [
            key,
            `postgresql://${role}:${randomToken()}@${pgHost}:${pgPort}/${db}`,
          ];
        }
...
export function applySecrets(
  ...
  for (const spec of secrets) {
    ...
    if (config.verbose) {
      console.log(
        `    Created ${spec.name}: ${Object.keys(spec.literals).join(", ")}`,
      );
    }
  }
}
