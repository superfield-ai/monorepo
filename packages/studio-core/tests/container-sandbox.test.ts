...
describe('container-sandbox', () => {
  ...
  describe('startSandbox', () => {
    ...
    it('labels the container for identification and cleanup', async () => {
      ...
      expect(createArgs).toContain('app=calypso-studio-sandbox');
    ...
    it('throws if container creation fails', async () => {
      ...
      const { startSandbox } = await import('../container-sandbox');
      expect(() => startSandbox(sandboxConfig)).toThrow('Failed to create sandbox container');
    });
    ...
    it('cleans up and throws if container start fails', async () => {
      ...
      const { startSandbox } = await import('../container-sandbox');
      expect(() => startSandbox(sandboxConfig)).toThrow('Failed to start sandbox container');
  ...
  describe('stopSandbox', () => {
    ...
    it('stops then force-removes the container', async () => {
      ...
      expect(spawnSyncMock.mock.calls[0][1]).toEqual(['stop', '--time', '5', 'studio-sandbox-a1b2']);
      expect(spawnSyncMock.mock.calls[1][0]).toBe('docker');
      expect(spawnSyncMock.mock.calls[1][1]).toEqual(['rm', '-f', 'studio-sandbox-a1b2']);
    });
  ...
  describe('listSandboxes', () => {
    ...
    it('parses docker ps output into SandboxInfo objects', async () => {
      spawnSyncMock.mockReturnValueOnce(mockSpawnResult(
        0,
    ...
    it('filters by the studio sandbox label', async () => {
      ...
      expect(args).toContain('label=app=calypso-studio-sandbox');
    });
  ...
  describe('cleanupOrphanedSandboxes', () => {
    ...
    it('stops and removes each orphan container', async () => {
      spawnSyncMock
        .mockReturnValueOnce(mockSpawnResult(0, 'abc|studio-sandbox-old1|Up 10m\n', ''))
        .mockReturnValueOnce(mockSpawnResult(0, '', ''))
        .mockReturnValueOnce(mockSpawnResult(0, '', ''));
  ...
  describe('buildAndExportImage', () => {
    ...
    it('runs docker build inside the sandbox container', async () => {
      ...
      buildAndExportImage(sandbox, 'calypso-release:studio');
    ...
    it('saves image to shared volume — not a network push', async () => {
      ...
      const outputArg = saveArgs.find((a: string) => a.includes('/studio/build-output/'));
    ...
    it('returns true on success', async () => {
      ...
      const { buildAndExportImage } = await import('../container-sandbox');
      expect(buildAndExportImage(sandbox, 'calypso-release:studio')).toBe(true);
    });
    ...
    it('returns false if build fails', async () => {
      ...
      const { buildAndExportImage } = await import('../container-sandbox');
      expect(buildAndExportImage(sandbox, 'calypso-release:studio')).toBe(false);
    });
