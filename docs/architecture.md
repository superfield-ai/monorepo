...
| `GET`        | `/analytics/check-runs/stream`  | Required         | `orchestrator` | SSE stream of CI/check-run events. **Producerless in v0**: the stream is plumbed but has no producer, so it is always empty. The UI also polls the non-stream `GET /analytics/check-runs?sha=` (`TurnTimeline.tsx`, `VisualDiffPanel.tsx`), which now has a backend route (#674, #713) |
