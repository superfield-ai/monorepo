...
| `WS`         | `/studio/ws`                    | Required | `ws`           | Agent-chat WebSocket; emits `chunk`/`done`/`error` frames. dev-scout stub emits one `chunk`+`done` then closes — real agent runtime is #687 (#695) |
