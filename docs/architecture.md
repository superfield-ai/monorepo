...
**Lifecycle (PRD §6):** a policy traverses `drafted → active → revised → retired`. The legal-transition graph (`drafted → active | retired`, `active → revised | retired`, `revised → active | retired`, `retired` terminal) is enforced by `PolicyState::can_transition_to` / `PolicyState::transition`, which are pure (no database) and unit-testable in isolation; the same vocabulary is pinned by a `CHECK` constraint in the migration. Only an `active` policy governs changes (`PolicyState::is_governing`). A partial unique index (`policies_one_active_per_workspace_idx`) allows at most one active policy per workspace.
