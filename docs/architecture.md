...
| 17  | Existing Nexum data not migrated to shared Postgres instance             | One-shot idempotent migration script (`packages/db/nexum-migration.ts`) copies blocks, embeddings, and links from the standalone nexum-pg container into the shared instance under a workspace key; nexum-pg Docker Compose service removed | Closed by #441 — see `packages/db/nexum-migration.ts` and `packages/db/migrations/nexum/`                                                            |
