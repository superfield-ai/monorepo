...
The Superfield Blueprint is the opinionated rule set that encodes how to build software correctly: architectural constraints, security principles, design patterns, checklists, and antipatterns. The governed target is a **versioned, fail-closed rule set held in the brain** (ratified 2026-07-02, red-team review R-03 — see the Governance decision below). An earlier framing described the Blueprint as Superfield's fine-tuned dev agent model with the rules baked into its weights; that framing is superseded as the target — fine-tuning is at most a possible future _encoding_ of the ratified rule set, not the thing the appliance governs. Weights cannot be a versioned rule set whose absence is a boot failure with per-change verdicts recorded in `forge.validation_runs`.

**Current implementation (interim).** Today the Blueprint is approximated as a compiled YAML rule graph sourced from `dot-matrix-labs/superfield-blueprint` and tracked as a git subtree at `blueprint/`. The compiled graph lives at `blueprint/rules/graph.yaml` (1 231 nodes across ARCH, AUTH, DATA, TEST, DEPLOY, ENV, PROCESS, UX, WORKER), with domain bodies under `blueprint/rules/blueprints/*.yaml` and TypeScript-specific implementation rules under `blueprint/rules/implementations/ts/`.
...
Nexum is distinct from the Blueprint: where the Blueprint defines the rules agents follow (the versioned rule set held in the brain), Nexum is the live company brain they reason against. The Blueprint tells an agent _how_ to build; Nexum tells it _what_ to build and _what is currently true_.
...
| Orchestrator | `orchestrator/migrations/` (`0001_gardening_cursor.sql` — applied by the appliance runner as the last `COMPONENT_DIRS` entry, #762; moves into a runner-walked `crates/` component directory when the prototype `orchestrator/` tree is archived per the 2026-07-02 archive decision) |
...
The canonical dependency order is **`sf-db → sf-auth → nexum → sharp → orchestrator`** — the order the Rust runner walks `COMPONENT_DIRS` (`crates/sf-db/src/migrate.rs`) and the daemon health gate applies. `sf-db` must run first because it creates `public.workspaces`, which component tables (e.g. `nexum.page_revisions`) FK against; the cross-cutting `orchestrator` cursor migration sorts last because it depends only on the component schemas existing (#762). This document owns the migration order; `adr-schema-boundary.md` defers to it by reference (its note that the runner does not walk `orchestrator/migrations/` predates #762 and is superseded here — the appliance runner does apply it, so the appliance creates its own cursor table).
...
**The two tracks (no longer split on enforcement):** the **k3s / TS-migrator** track applies `packages/db/migrations` via `packages/db/migrator.ts` (the standalone k8s-Job migrator, never imported by the app server). The **appliance daemon** applies its mirror through the Rust migration runner, which walks `COMPONENT_DIRS` (`crates/sf-db/src/migrate.rs`) in `sf-db → sf-auth → nexum → sharp → orchestrator` order; the RLS file is mirrored into the last _schema-owning_ component directory as its highest-numbered file (`crates/sharp/migrations/0009_rls_workspace_isolation.sql`) so it runs after every RLS-scoped schema exists, matching the TS migrator's "RLS applied last" ordering (the cross-cutting `orchestrator` cursor migration sorts after it and creates no RLS-scoped tables, #762). Because sf-db's `0003` workspace_id threading runs before the component tables exist on the appliance order, the appliance RLS migration also self-ensures the `workspace_id` column on each target table before enabling RLS. So on the running self-provisioning appliance, cross-workspace reads/writes are now denied by the database (issue #710).
...
**Read route:** `GET /studio/changes/{id}` (`crates/sf-serve/src/routes/change.rs`, auth-required) returns a change and its current state. Change-content generation is out of scope for this state machine and tracked separately; the policy risk evaluation it once deferred now lives in the policy engine below.

### Policy engine — autonomous vs approval

The Policy entity (PRD US4 and §3 — the Owner "sets policy: what risk level may ship without human review, and what requires sign-off") owns the autonomous-versus-approval decision the change-merge gate consults. It lives in `crates/sf-db/src/policy.rs` (entity + persistence) over `forge.policies` (`crates/sf-db/migrations/0005_policy_engine.sql`).

**Lifecycle:** a policy traverses `drafted → active → revised → retired` in the database vocabulary. PRD §6 names the first state "template selected" (policy is never authored from scratch); the DB's `drafted` maps to it — a certified template selected and tuned, not yet active. The legal-transition graph (`drafted → active | retired`, `active → revised | retired`, `revised → active | retired`, `retired` terminal) is enforced by `PolicyState::can_transition_to` / `PolicyState::transition`, which are pure (no database) and unit-testable in isolation; the same vocabulary is pinned by a `CHECK` constraint in the migration. Only an `active` policy governs changes (`PolicyState::is_governing`). A partial unique index (`policies_one_active_per_workspace_idx`) allows at most one active policy per workspace.

**Risk and the merge gate (PRD Constraint §9):** `RiskLevel` is a `0..=100` score for a change's blast radius, on the same scale as a policy's `risk_threshold`. `Policy::evaluate(risk)` is pure and total: it returns `MergeDecision::Autonomous` only when the policy is governing **and** permits autonomy **and** the change's risk is at or below the threshold; otherwise `MergeDecision::RequiresApproval`. The persisted merge-gate entrypoint `evaluate_change_against_active_policy` resolves the single `active` policy for a workspace and applies the identical rule, so the two paths agree by construction and the merge path cannot auto-ship a change above the active threshold. When **no** policy is `active`, the gate is **fail-closed**: every change requires approval, so a missing policy never silently grants autonomy. A change routed to `RequiresApproval` is what drives the lifecycle into `awaiting-approval` (and, in turn, the outbound approval notification below).
...
## Outbound Notifications — `sf-notify`

PRD §7 requires alerting a human when a change awaits approval or a runtime signal demands attention. The `crates/sf-notify` crate is that outbound seam — it owns _delivery_, not the decision of _when_ to alert. The governance/policy layer decides a change should await approval (see §Policy engine) or that a signal is worth a human's attention, then calls the `Notifier`.

**Transport seam:** `NotificationChannel` is a `Send + Sync` transport trait returning a boxed future, mirroring the established trait-seam style elsewhere in the workspace (`sf_loop::agent::AgentExecutor`, `sf_db::provisioner::PostgresProvisioner`). Two implementations ship: `WebhookChannel` (the real transport) and `InMemoryChannel` (a test double that captures every dispatched `Notification` for assertions).

**Dispatcher (`Notifier`):** turns the two product triggers into `Notification`s:

- `Notifier::notify_awaiting_approval` — **always** dispatches. Reaching `awaiting-approval` (`sf_db::change::ChangeState::AwaitingApproval`) is itself the human-in-the-loop trigger, so there is no severity gate here.
- `Notifier::notify_signal` — dispatches **only** for high-severity signals (`SignalSeverity::is_high`, i.e. `High` or `Critical`). A `Low`/`Medium` signal is dropped at the channel so a flood of routine production noise never pages a human.

**Severity is a notification-layer concept, not a signal-store column.** `SignalSeverity` lives in `sf-notify`, not on Sharp's `sharp.runtime_signals` schema. A caller maps whatever it knows (signal kind, payload, policy verdict) to a `SignalSeverity` and passes it in; this keeps the channel decoupled from the signal store and is the only severity judgement the channel makes — richer routing belongs in the policy engine, not here.

---

## Systems-of-Record Connector Seam — `sf-connector`

PRD US18 / §7 ("read from systems of record") requires a new Superfield app to read data the enterprise already maintains in its existing systems of record (CRM, ERP, ticketing, …) _without modifying or replacing those systems_ (a PRD §Non-Goal). The `crates/sf-connector` crate defines that seam.

**Read-only by construction.** The `Connector` trait (`Send + Sync`) exposes only read verbs — `query`, `fetch`, and the introspection helpers `source_name` / `resources`. There is deliberately **no** `insert` / `update` / `delete` / `write` / `sync` method anywhere in the trait, so the read-only guarantee is structural rather than merely documented; a `connector_seam` test asserts the trait exposes only read methods so a write-shaped addition is caught at compile/test time. Reads are on demand: an app calls `query`/`fetch` when it needs data and the seam returns the rows it read.

**Per-workspace credentials.** `ConnectorCredentials` is scoped to exactly one workspace via `workspace_id` (keyed by `workspaces.id`, the same per-tenant key used across the substrate). There is no cross-workspace or global credential variant — a credential minted for workspace A can never be presented for workspace B, because the reference connector binds the credential's `workspace_id` at construction and the seam offers no way to retarget it. `CredentialKind` is a closed schema (`ApiKey`, `BasicAuth`, `OAuthBearer`) rather than an opaque blob, so the governance layer can reason about credential types per workspace without inspecting the secret material.

**Reference connector.** `InMemoryConnector` holds an immutable in-memory `FakeSource` table set and serves reads from it; it never mutates its backing source on any call. It is the connector used by tests and by component crates that need the seam without a live external system.
...
2. Run the migration runner (`sf_db::run_migrations`) against the now-live instance — applies every component schema in `sf-db → sf-auth → nexum → sharp → orchestrator` order, idempotently, tracked in `schema_migrations`. The final `orchestrator` entry creates `orchestrator.gardening_cursor`, so the gardening loop's resumable cursor exists before the loop starts.
...
The loop's resumable position is stored in `orchestrator.gardening_cursor` (one row per workspace). The table is created by `orchestrator/migrations/0001_gardening_cursor.sql`, which the appliance migration runner applies as the final `COMPONENT_DIRS` entry (#762 — see §Migration ownership); when the prototype `orchestrator/` tree is archived (ratified 2026-07-02), that migration moves into a runner-walked `crates/` component directory with the schema and table names unchanged.
...
The `role` carried by the `AuthContext` is one of seven implemented roles (`owner`, `requestor`, `steerer`, `collaborator`, `agent`, `auditor`, `viewer`; the legacy `admin`/`member` names map onto `owner`/`collaborator`) — seven of PRD §3's **nine**: **Administrator** and **Partner operator (MSP/VAR)** have no auth-role mapping yet, and the PRD §7 partner fleet-management surface is unbuilt and has no route representation here (it is control-plane-only by decision — 2026-07-03 open-tensions review, D-2). On top of authentication, protected routes enforce **route-level authorization** in `sf_serve::authz`: write routes are gated by `require_write` (the read-only `auditor` and `viewer` roles receive `403 Forbidden`), and Owner-only governance routes — starting/stopping the autonomous loop — are gated by `require_owner` (every non-Owner role receives `403`). The read-only **auditor query surface** (`/studio/audit/*`, `crates/sf-serve/src/routes/auditor.rs`) is gated tighter still: it is granted to exactly `Role::Auditor` (the compliance reviewer) and `Role::Owner`, with every other role — including the read-only `viewer` and all read/write roles — denied `403` (#719). These gates run after the auth middleware and complement the per-schema RLS policies that isolate workspace data at the database.
...
| `GET`        | `/studio/audit/decisions/{id}`  | Required + Owner/Auditor | `auditor`      | Read-only causal chain for a decision/change: `decision → error → session → user → requirement → code`, projected from the property graph via `nexum::decision_to_requirement_chain`. Gated to `Role::Auditor`/`Role::Owner`; every other role (including `viewer`) gets `403`. Sparse causal data returns `200` with a `causal: null` / `missing_hops` envelope, not an error (#719) |
| `GET`        | `/studio/cluster/events`        | Required                 | `cluster`      | SSE stream of cluster-status transitions driving the live-preview reload. A **continuous health poller** (`crates/superfield/src/daemon_runtime.rs`, #715) observes the preview workload on a bounded interval and publishes a transition only on change, so a mid-session `restarting→healthy` flip after a rebuild IS pushed in production                                          |
...
| `GET`        | `/analytics/loops`              | Required                 | `orchestrator` | Per-loop health for the plan/dev/doc lanes. The gardening loop maps each step to a `plan`/`dev`/`doc` lane and records per-lane ticks, so all three lanes report live health (#712)                                                                                                                                                                                                   |
| `GET`        | `/analytics/slots`              | Required                 | `orchestrator` | Active work slots driving the Orchestrator cards. The gardening loop publishes a real `WorkSlot` per running step, carrying an accumulating `costUsd` field (#712)                                                                                                                                                                                                                    |
| `GET`        | `/analytics/check-runs`         | Required                 | `orchestrator` | Non-stream CI check-runs for one commit: `?sha=<sha>` returns the `{ checkRuns: [...] }` envelope (`CheckRunsResponse`) that `TurnTimeline.tsx` and `VisualDiffPanel.tsx` poll. **Producerless in v0**: the check-run producer (A7) is a separate feature, so the envelope is well-formed but `checkRuns` is always empty for now (#713)                                              |
| `GET`        | `/analytics/check-runs/stream`  | Required                 | `orchestrator` | SSE stream of CI/check-run events. The gardening loop emits a real check-run event per step completion (#712). The UI also polls the non-stream `GET /analytics/check-runs?sha=` (`TurnTimeline.tsx`, `VisualDiffPanel.tsx`), which now has a backend route (#674, #713)                                                                                                              |
...
├── auditor.rs     — GET /studio/audit/* (auth required, Owner/Auditor only — read-only decision-trace API) (#719)
...
- **`/health` is a bare liveness probe, not a readiness check** — the handler verifies nothing beyond the process serving HTTP. Readiness comes from boot ordering rather than the route: the TCP listener binds only after the health-gated boot completes **and** the gardening loop has started (Postgres accepting connections, all migrations applied — see §Daemon Lifecycle and `boot.rs::health_gate` — plus the loop task spawned with its real `LoopHandle` installed in `AppState`: `daemon_runtime::boot_loop` runs before the bind in `run_as_daemon`, `crates/superfield/src/main.rs`). A reachable `/health` therefore implies all three gates passed at startup — the same three bullets milestone-1.md §4.5 lists, including its loop-registration bullet, which this boot order backs. It does not re-verify them per request; milestone-1.md's readiness description refers to this bind-after-boot guarantee, not to per-request checks.
...
- **Analytics producers are wired from the gardening loop (#712).** Running with an observer, the loop publishes a real `WorkSlot` per step (with an accumulating `costUsd` derived from the agent executor's reported cost), records per-lane (`plan`/`dev`/`doc`) ticks so `/analytics/loops` reports live health on all three lanes, and emits a check-run event per step completion on `/analytics/check-runs/stream`. The cost source is the `AgentExecutor` (`LlmAgentExecutor` derives it from token usage; `FixtureAgentExecutor` returns a fixed non-zero cost). The non-stream `GET /analytics/check-runs?sha=` route remains unbacked (out of scope for #712).
- **`/studio/cluster/events` is driven by a continuous health poller (#715).** Beyond the once-at-boot seed, a bounded-interval poller (`run_cluster_status_poller` in `crates/superfield/src/daemon_runtime.rs`) observes the preview workload's health off the shared supervisor and publishes a `cluster-status` transition only when the mapped status changes, so a mid-session `restarting→healthy` flip after a rebuild is pushed to the stream and drives the control-panel `IframePanel` reload without a manual refresh.
...
The deployment tier is driven by the **engine-agnostic `FastenvManifest`**. The Rust `deployment::FastenvManifest` is the **canonical** definition of the wire shape (ratified 2026-07-02, archive-the-prototype decision: Rust canonical, with the TS artifact generated or deleted). The TypeScript emitter `packages/control-core/fastenv-translate.ts` (which translates Kubernetes manifests / docker-compose into the engine-agnostic spec) is archived-prototype interim — it survives only until the `packages/*` tree is archived, kept in sync field-for-field with JSON round-trip / contract tests on both sides in the meantime. The supervisor reads each `Workload`'s `name`, `image`, `command`, `env`, `stateful`, and optional `health` probe. A `Workload` with `stateful: true` is the StatefulSet equivalent (Postgres), an in-process service registry plus host-local addressing replaces kube-proxy + CoreDNS, and `HealthProbe` is consumed by the `doctor` readiness surface.

### Backend selector and doctor

**fastenv is the default deploy backend** (ratified 2026-07-02, archive-the-prototype decision). The TS selector `runDeployCommand` (`packages/core/commands/deploy.ts`) — which takes a `backend: "k3s" | "fastenv"` option and still hardcodes `k3s` as its default (CLI flag `superfield deploy --backend fastenv`) — is archived-prototype interim: its k3s path and its default are slated for archival with the rest of `packages/*` and are not appliance architecture. On the fastenv backend it translates `deploy/base/*.yaml` into a `FastenvManifest`, runs `fastenv doctor` for provision, then `fastenv up --manifest … --health-gate` for deploy + readiness — and the recorded command trace contains **only `fastenv` commands** (issue #660 criterion 4). `fastenv doctor` (the subcommand that shipped with Milestone 1, see below) is the host-prerequisite preflight: it verifies KVM, CPU virtualisation flags, the Firecracker and `crun` binaries, TUN/TAP, overlayfs, and kernel version before any VM lifecycle command is attempted.
