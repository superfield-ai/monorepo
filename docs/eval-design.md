...
covers the part that is *not* deterministic: whether the loop, driven by a real
...
> criteria, run through the *real* loop, scored over N runs to a pass
> threshold.
...
eval ledger. The work is largely to *grade* it, not to build new recording.
...
We make each acceptance criterion an *executable behavioral assertion* attached
to a Feature, one of:
...
\* Tier 1 replays recorded artifacts, so the *inputs* are fixed; an LLM-judge
...
*plumbing*, not the *intelligence*. Runs on every PR.

### Tier 1 — Step / artifact graders (build first; cheapest signal)

Replay a recorded episode and score a *single* gardening step's output against a
rubric:
...
run the *entire* loop against a live model, then assert the outcome:
...
- **Score statistically:** run each scenario *k* times, require pass-rate ≥ a
  threshold, and use multi-sample judges rather than a single verdict. Track the
  *distribution*, not one lucky run.
- **Separate outcome from process metrics** so a regression tells you *where* it
  broke:
...
- **Acceptance, scoped to today's capability** (see
  ...
  produced a *compiling* candidate (`merge_result` recorded). A Playwright smoke
