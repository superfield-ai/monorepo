...
_Unified:_ the 500 is written as an event row the moment it fires, with foreign keys to the session, the cohort, the features, their requirements, and the open issues against them. Within seconds an agent traverses error → session → feature → requirement → open issues → prior attempts → current code → test coverage, produces a grounded diagnosis, and — if confidence is high and risk is low — opens a draft patch. The human sees: _"Checkout 500 affecting 3 sessions in the last hour. Likely cause: null pointer in payment callback. Draft patch ready for review."_ The human reviews the patch, not the diagnosis — the diagnosis used more context than a person could assemble by hand.
