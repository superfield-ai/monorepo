...
- **Gap surfacing** — the brain's synthesized view of the business, used to spot operational gaps that become the next proposed apps. The view's currency is bounded honestly: it is **current for what the brain governs** (the apps, signal, and intent living in the store, stamped on the brain's clock) and **timestamped for what it reads** (external systems of record update outside the brain's clock, so connector reads carry freshness metadata rather than a currency guarantee). Accordingly, gap surfacing's v1 input is scoped to in-brain app signal; gaps inferred from the timestamped external view are surfaced explicitly as lower-confidence. This is the expansion engine of the green wedge: it is how one app becomes ten without a sales motion.
...
The scaffold bakes in: the shared authentication layer (§2.10), the telemetry SDK (§2.7), a data layer that reads the brain and — through governed, read-only connectors — the enterprise's existing systems of record, without modifying them. The connector boundary question splits into two halves with different owners. The **live-read-vs-copy semantics and freshness governance** — which data an app reads live, what may be copied into the brain, and how both are labeled (every external read timestamped in the brain's clock domain, staleness a first-class property surfaced to agents rather than an unmarked fact) — is a product/architecture question **owned and answered here, in this section**, because what may be copied into the brain determines what the one-schema/one-clock coherence guarantee covers. The **data-residency and regulatory** half of the boundary is deferred to the commercial workstream, as the PRD's open-questions section records; that deferral does not cover the copy semantics, which no commercial workstream can answer.
...
- **The control panel** — where humans steer. The primary review mode is **batch review of completed candidates**: agents run to completion and present finished candidate states — variants, before-and-after demonstrations against representative data, policy exceptions — and the human approves or rejects at the **outcome level**, judging demonstrated behavior with the full reasoning chain in one place (intent, triggering signal, affected users, validation, plain-language risk statement), never code diffs. An interim live mode — watching an agent work against a live preview and correcting it mid-task — remains available while agent runs are slow enough for it to be useful, and is expected to recede as agent speed grows. The panel also covers: stating intent; monitoring every active agent's task, elapsed time, and cost; reviewing components and screens against representative data across device sizes; overseeing deployment health and rolling back.

And the identity beneath both: enterprise SSO mapped to the PRD's nine-role model (Owner, Administrator, Requestor, Steerer, Collaborator, Partner operator, Agent, Auditor, Viewer), with agents as first-class principals so policy and audit apply to them uniformly, and with a person's permissions identical on both surfaces. The Partner operator additionally acts through the multi-appliance fleet-management surface required by PRD §7, which is a partner-facing surface distinct from the two per-app surfaces above.
...
7. **The Forge itself into fastenv** (§2.3 completing; PRD §9 "fastenv execution (target state)") — re-host the Forge as a fastenv workload on the same fabric that already runs its validation jobs and delivered apps, retiring the last workload outside the execution boundary. Validation jobs and apps carry the fastenv requirement from step 4 onward; until this step lands, the constraint remains a **target state** for the Forge itself, not a present fact. This is the "scheduled later step" the PRD references, and it also closes the PRD's open question about Forge/app isolation on a shared host.
