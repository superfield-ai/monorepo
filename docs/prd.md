...
- Serving as a general-purpose data warehouse or analytics platform for retrospective reporting unrelated to the development loop.
...
- **Validation gate.** No change may merge without passing validation against a current baseline, and no change above the policy-defined risk threshold may ship without human approval.
