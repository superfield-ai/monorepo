...
//
// The tests live in `mod gha_adapter { mod tests { … } }` so nextest reports
// them at the exact AC path `gha_adapter::tests::<name>`, and the bare
// positional filter in `cargo nextest run -p fastenv gha_adapter
// --no-tests=fail` (which matches against the TEST NAME, not the binary id)
// selects them — running >0 tests instead of collecting zero.
//
...
mod gha_adapter {
    mod tests {
        ...
        /// equivalence on re-emit. Because every real workflow uses constructs
        /// the substrate-agnostic manifest deliberately does not model
        /// (marketplace `uses:`, matrices, reusable workflows, `${{ }}`
        /// contexts), the assertion is the NO-SILENT-DROP contract: each file's
        /// import is EITHER Ok whose emit re-imports to an equal manifest, OR a
        /// loud Err naming the unsupported construct — never a silent partial.
        /// This guards that a real construct the adapter cannot represent fails
        /// CI instead of being dropped.
        #[test]
        ...
        /// AC (invariant 1): each one-construct snippet must return a clear Err
        /// that NAMES the construct; NONE may return Ok (a silent drop would be
        /// a false green).
        #[test]
        ...
        /// independent of fixtures: default contract, no gate, jobs sorted by
        /// id, argv with no whitespace/metacharacters.
        #[test]
        ...
        /// end-to-end, asserting success and that the produced manifest matches
        /// the golden one. Exercises the new subcommands without touching
        /// main.rs tests.
        #[test]
