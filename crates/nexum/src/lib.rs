...
//!
//! All operations run in-process against the shared Postgres instance via a
//! supplied `sqlx::PgPool` and the governed 384-dim embedding model via the
//! inlined `crate::embed` module.
//!
//! # Pipeline overview
//!
//! 1. **Parse** — split a document into typed blocks (`parse` module).
//! 2. **Dedup** — SHA-256 content hash each block (`dedup` module).
//! 3. **Embed** — produce 384-dim vectors via `crate::embed::Embedder` (`ingest` module).
