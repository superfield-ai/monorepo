...
/// Block row loaded for a version: id + content + owning workspace.
struct BlockRow {
    id: Uuid,
    content: String,
    workspace_id: Uuid,
...
pub async fn process_ai_links(
    ...
    //    resolve the corpus).
    let blocks: Vec<(Uuid, String, Uuid, Uuid)> = sqlx::query_as(
        r#"
        SELECT b.id, b.content, b.doc_id, b.workspace_id
        FROM nexum.blocks b
    ...
    let blocks: Vec<BlockRow> = blocks
        .into_iter()
        .map(|(id, content, _, workspace_id)| BlockRow {
            id,
            content,
            workspace_id,
        })
    ...
    for block in &blocks {
        ...
        for cand in similar {
            ...
            let edge_vec =
                embed_edge(embedder, &block.content, &candidate.content, Some(rel_type))?;
            ...
            sqlx::query(
                r#"
                INSERT INTO nexum.links (id, workspace_id, src, dst, layer, rel_type, weight, confirmed, provenance, edge_embedding)
                VALUES ($1, $2, $3, $4, 'ai', $5, $6, NULL, $7::jsonb, $8::vector)
...
mod tests {
    ...
    fn below_threshold_returns_none() {
        assert_eq!(
            classify_pair("similarly, this also confirms it", 0.69),
            None
        );
    }
    ...
    fn high_sim_no_keyword_falls_back_to_supports() {
        assert_eq!(
            classify_pair("plain unrelated text", 0.86),
            Some("supports")
        );
    }
